...
const page = () => {
  ...
  const fetchTasks = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/admin/get_all_tasks', {
  ...
  const fetchUsers = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/admin/get_all_users', {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  const deleteTask = async (taskId) => {
    try {
      await axios.delete(
        'http://localhost:5000/api/admin/delete_task',
        { taskId },
        { headers: { Authorization: `Bearer ${token}` } }
      )
